...
from ..db import get_all_tracks_with_counts, get_top_artists, query_all_dbs
...
def generate_custom_playlist(
    ...
    Args:
        ...
        min_plays: Minimum play count for tracks
        max_days: Maximum days since last play
        include_unplayed: Whether to include tracks from Spotify search
    ...
    # If including unplayed and we need more tracks
    if include_unplayed and len(result) < limit and genres_lower:
    ...
    # Enrich tracks from history with Spotify data
    history_tracks = [t for t in result if not t.get("from_search")]
    if history_tracks:
